...
public class PlayerControlPanel extends AnchorPane {
	...
	public PlayerControlPanel(AudioPlayer player) {
		...
		getStylesheets().add("com/sharpkoi/oiduark/app/style/player-control-panel.css");
	...
	public Button playButton() {
		if(b_play == null) {
			ImageView playIcon = new ImageView(ResourceLoader.loadPlayIcon());
			playIcon.setFitHeight(32);
			playIcon.setFitWidth(32);
			b_play = new Button();
			b_play.setGraphic(playIcon);
	...
	public Button lastButton() {
		if(b_last == null) {
			ImageView lastIcon = new ImageView(ResourceLoader.loadLastIcon());
			lastIcon.setFitHeight(24);
			lastIcon.setFitWidth(24);
			b_last = new Button();
			b_last.setGraphic(lastIcon);
	...
	public Button nextButton() {
		if(b_next == null) {
			ImageView nextIcon = new ImageView(ResourceLoader.loadNextIcon());
			nextIcon.setFitHeight(24);
			nextIcon.setFitWidth(24);
			b_next = new Button();
			b_next.setGraphic(nextIcon);
	...
	public void onPlayerStop() {
		...
		l_timeTick.setText("-:--");
		((ImageView) b_play.getGraphic()).setImage(ResourceLoader.loadPlayIcon());
		disable();
	}
		...
		b_play.setOnMouseClicked(e -> {
			if(player.isPlaying()) {
				...
				if(player.indicator == -1) {
					if(player.play()) {
						((ImageView) b_play.getGraphic()).setImage(ResourceLoader.loadPauseIcon()); 
					}
				}else {
					player.resume();
					((ImageView) b_play.getGraphic()).setImage(ResourceLoader.loadPauseIcon());
